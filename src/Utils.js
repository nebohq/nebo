...
const breakWords = (input, withWord = (word) => word) => {
  ...
  return words.filter(Boolean).map(wordModifier);
};

const humanize = (input, titleize = false) => {
  const withWord = titleize ? capitalize : (word) => word;
  return capitalize(breakWords(input, withWord).join(' '));
};

const kebabCase = (input) => breakWords(input).join('-');
...
export {
  ...
  evaluate, isNullish, humanize, capitalize, kebabCase, neboURL, componentURL, fetchComponent,
};
